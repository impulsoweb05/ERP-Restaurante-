...
exports.handleLevel2 = handleLevel2;
exports.handleLevel3 = handleLevel3;
exports.handleLevel4 = handleLevel4;
exports.handleLevel5 = handleLevel5;
exports.handleLevel6 = handleLevel6;
exports.handleLevel7to13 = handleLevel7to13;
exports.handleLevel14 = handleLevel14;
exports.handleLevel15 = handleLevel15;
...
// ═══════════════════════════════════════════════════════════════════════════
async function handleLevel2(session, message) {
    try {
        ...
        logger_1.logger.error('Error in level 2', error);
        return {
...
async function handleLevel3(session, message) {
    try {
        if (!session.selected_category) {
            return {
                message: 'Error: No hay categoría seleccionada.',
                session: await SessionService_1.SessionService.updateLevel(session.session_id, 2)
            };
        }
        const subcategories = await MenuService_1.MenuService.getSubcategoriesByCategory(session.selected_category);
        ...
        logger_1.logger.error('Error in level 3', error);
        return {
...
async function handleLevel4(session, message) {
    try {
        if (!session.selected_subcategory) {
            return {
                message: 'Error: No hay subcategoría seleccionada.',
                session: await SessionService_1.SessionService.updateLevel(session.session_id, 3)
            };
        }
        const items = await MenuService_1.MenuService.getItemsBySubcategory(session.selected_subcategory);
        if (items.length === 0) {
            return {
        ...
        logger_1.logger.error('Error in level 4', error);
        return {
...
// ═══════════════════════════════════════════════════════════════════════════
async function handleLevel5(session, message) {
    try {
        if (!session.temp_menu_item) {
            return {
                message: 'Error: No hay producto seleccionado.',
                session: await SessionService_1.SessionService.updateLevel(session.session_id, 4)
            };
        }
        // Obtener producto
        const item = await MenuService_1.MenuService.getItemById(session.temp_menu_item);
        if (!item) {
            return {
...
Volviendo al menú...`,
        ...
        logger_1.logger.error('Error in level 5', error);
        return {
...
// ═══════════════════════════════════════════════════════════════════════════
async function handleLevel6(session, message) {
    try {
        const cart = await CartService_1.CartService.getCart(session.session_id);
        if (cart.length === 0) {
            return {
...
2️⃣ Salir`,
        ...
        const totals = CartService_1.CartService.calculateTotals(cart);
        ...
        logger_1.logger.error('Error in level 6', error);
        return {
...
// ═══════════════════════════════════════════════════════════════════════════
async function handleLevel7to13(session, message) {
    try {
...
Escribe tus comentarios o escribe "ninguno" para continuar:`,
        ...
        if (level === 13) {
            ...
            const nextSession = await SessionService_1.SessionService.updateLevel(session.session_id, 14);
            return {
        ...
        const nextSession = await SessionService_1.SessionService.updateLevel(session.session_id, level + 1);
        return {
            message: 'Continuando...',
            session: nextSession
        };
        ...
        logger_1.logger.error('Error in checkout levels', error);
        return {
...
// ═══════════════════════════════════════════════════════════════════════════
async function handleLevel14(session, message) {
    try {
        const cart = await CartService_1.CartService.getCart(session.session_id);
        if (cart.length === 0) {
            return {
...
No podemos procesar tu pedido.`,
        ...
        for (const item of cart) {
            const menuItem = await MenuService_1.MenuService.getItemById(item.menu_item_id);
            if (!menuItem || menuItem.status !== 'active') {
                return {
                    message: `❌ El producto "${item.name}" ya no está disponible.

Por favor revisa tu carrito.`,
                    session: await SessionService_1.SessionService.updateLevel(session.session_id, 6)
                };
            }
        }
        const totals = CartService_1.CartService.calculateTotals(cart);
        const checkoutData = session.checkout_data;
...
2️⃣ NO, Cancelar`;
        ...
        logger_1.logger.error('Error in level 14', error);
        return {
...
// ═══════════════════════════════════════════════════════════════════════════
async function handleLevel15(session, message) {
    try {
        const choice = message.trim();
        // Cancelar
        if (choice === '2' || choice.toLowerCase().includes('no') || choice.toLowerCase().includes('cancelar')) {
            await CartService_1.CartService.clearCart(session.session_id);
            return {
                message: 'Pedido cancelado. ¡Hasta pronto!',
                session: await SessionService_1.SessionService.updateLevel(session.session_id, 0)
            };
        }
        // Confirmar
        ...
        if (!session.customer_id) {
            return {
                ...
                session: await SessionService_1.SessionService.updateLevel(session.session_id, 1)
            };
        }
        ...
        const { order, items } = await OrderService_1.OrderService.createOrder({
        ...
        await SessionService_1.SessionService.updateLevel(session.session_id, 0);
...
¡Gracias por tu preferencia!`;
        return {
            ...
            session: await SessionService_1.SessionService.getOrCreateSession(session.session_id),
            data: { order, items }
        };
    }
    catch (error) {
        logger_1.logger.error('Error creating order in level 15', error);
        return {
