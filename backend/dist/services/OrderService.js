...
class OrderService {
    ...
    static async createOrder(data) {
        ...
        try {
            ...
            for (const item of data.order_items) {
                ...
                const menuItemResult = await client.query('SELECT price, delivery_cost FROM menu_items WHERE id = $1 AND status = $2', [item.menu_item_id, 'active']);
         ...
         RETURNING *`, [
           ...
           RETURNING *`, [
                ...
                await client.query(`INSERT INTO kitchen_queue
           (order_item_id, priority, status, estimated_time)
           VALUES ($1, 3, 'queued', $2)`, [itemResult.rows[0].id, 15] // 15 min default
                );
            }
            await client.query('COMMIT');
            logger_1.logger.info('Order created', { order_number, order_id: order.id, items: orderItems.length });
            // Notificar nuevo pedido via WebSocket
            WebSocketService_1.WebSocketService.notifyNewOrder(order.id, order.order_number, order.customer_id);
    ...
    static async getOrderById(orderId) {
        try {
            ...
            const itemsResult = await (0, database_1.query)('SELECT * FROM order_items WHERE order_id = $1', [orderId]);
            return {
            ...
            logger_1.logger.error('Error getting order by id', error);
            throw error;
        }
    }
    /**
    ...
    static async updateStatus(orderId, status) {
        try {
            ...
            const result = await (0, database_1.query)(query_text, values);
            ...
            WebSocketService_1.WebSocketService.notifyOrderStatusUpdate(updatedOrder.id, updatedOrder.order_number, status, updatedOrder.customer_id, updatedOrder.waiter_id);
            return updatedOrder;
        }
        catch (error) {
     ...
     */
    static async getOrdersByCustomer(customerId, limit = 10) {
        try {
         ...
         LIMIT $2`, [customerId, limit]);
            ...
            logger_1.logger.error('Error getting orders by customer', error);
            throw error;
        }
    }
    // Alias para routes
